	...
	private void reload() {
		...
		VolleyRequest.GetCookieRequest(IndentDetailsActivity.this, link, new VolleyRequest.HttpStringRequsetCallBack() {
			...
			public void onFail(String error) {
				CustomToast.makeToast(IndentDetailsActivity.this, error, Toast.LENGTH_SHORT);
				i_dismissProgressDialog();
				finish();
			}
		});
...
//				 http://www.fanerjewelry.com/mobile/index.php?act=member_order&op=order_cancel&orderNo=7000000000015701&key=ab3b185aaade2ce3b15f9def978a8ad0
				String cancelLink = NewLink.CANCEL_ORDER_LINK + "&orderNo=" + orderID + "&key=" + Utils.getTokenKey(app);
...
//						JSONArray jsonArray = new JSONObject(result).getJSONObject("data").getJSONArray("buyPros")
						JsonElement elm = gson.fromJson(result, JsonElement.class).getAsJsonObject().get("data");
